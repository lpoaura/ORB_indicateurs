create materialized view orb_indicateurs.mv_sraddet_ind_pole_geom as (
select    la.geom,																				-- maille 5x5
		  la.id_area, 																			-- id maille
		  case when  t.regne = 'Animalia' and t.phylum != 'Chordata' then 'Invertébrés'
	 	 ...
	 	 case when EXTRACT(YEAR FROM s.date_min) BETWEEN 2001 AND 2005 then '2001 - 2005'
	 	  ...
	 	  when EXTRACT(YEAR FROM s.date_min) BETWEEN 2021 AND 2025 then '2021 - 2025' end as annee_group,												-- année d'observation 
...
inner join lpoaura_afo.maille_reg la on cas.id_area = la.id_area 																		-- filtre sur les maille 5kmx5km 
